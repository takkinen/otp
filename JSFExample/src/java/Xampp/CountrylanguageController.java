...
import Xampp.util.JsfUtil;
import Xampp.util.PaginationHelper;
...
public class CountrylanguageController implements Serializable {
    ...
    @EJB
    private Xampp.CountrylanguageFacade ejbFacade;
    ...
    public Countrylanguage getSelected() {
        if (current == null) {
            ...
            current.setCountrylanguagePK(new Xampp.CountrylanguagePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public Countrylanguage getCountrylanguage(Xampp.CountrylanguagePK id) {
    ...
    public static class CountrylanguageControllerConverter implements Converter {
        ...
        Xampp.CountrylanguagePK getKey(String value) {
            Xampp.CountrylanguagePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new Xampp.CountrylanguagePK();
            key.setCountryCode(values[0]);
            key.setLanguage(values[1]);
            return key;
        }

        String getStringKey(Xampp.CountrylanguagePK value) {
